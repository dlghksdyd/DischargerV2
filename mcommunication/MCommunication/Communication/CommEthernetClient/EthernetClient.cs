...
namespace Ethernet.Client.Common
{
    ...
    {
        public static int MAX_DATA_LENGTH = 1460;
    }
    ...
    public class EthernetClientStart
    {
        ...
        public bool CheckFunction = true;
    }
    ...
    public class EthernetClient
    {
        ...
        {
            EthernetClientConstant.MAX_DATA_LENGTH = maxDataLength;
        }
        ...
        public bool Query(byte[] _writeBuffer)
        {
            lock (PacketLock)
            {
                byte[] readBuffer = new byte[0];
                ...
                bool parseResult = Parameters.ParseFunction.Invoke(readBuffer, _writeBuffer);
                if (parseResult == false)
                {
                    return false;
                }
        ...
        public bool Command(byte[] _writeBuffer)
        {
            lock (PacketLock)
            {
                bool writeResult = Parameters.WriteFunction.Invoke(Handle, _writeBuffer);
